...
const boreholeSchema = z
  .object({
    ...
    field_energy_percent: z
      ...
      .max(100, "Energía máx 100%"), // Deleted rod_length from the next linecode
    water_table_depth: z
      .number()
      .min(0, "Profundidad >= 0")
      .optional()
  ...
  .refine(
    (data) => {
      ...
      if (assignments[0].depth_from !== 0) {
        console.error(
          `❌ First stratum must start at 0, but starts at ${assignments[0].depth_from}`
        );
        return false;
      }
...
const BoreholesConfigurationForm: React.FC = () => {
  ...
  const fixDepths = (boreholeIndex: number) => {
    ...
    const sorted = [...currentAssignments].sort(
      (a, b) => a.depth_from - b.depth_from
    );
        ...
        {boreholeFields.map((field, boreholeIndex) => (
          <div
            ...
            className={`${styles.tabContent} ${
              currentTab === boreholeIndex ? styles.active : styles.hidden
            }`}
          >
      ...
      <div className={styles.profileContainer}>
          ...
          <div className={styles.descriptionColumn}>
            ...
            {boreholeData.assignments.map((assignment, index) => {
              const stratum = availableStrata.find(
                (s) => s.name === assignment.stratum_code
              );
              return (
        ...
        {boreholeData.waterTable && (
          <div
            ...
            style={{
              top: `${
                (boreholeData.waterTable / boreholeData.finalDepth) * 200 + 40
              }px`,
            }}
          >
