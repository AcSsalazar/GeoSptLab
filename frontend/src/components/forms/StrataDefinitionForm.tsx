...
const stratumDefinitionSchema = z.object({
  stratum_code: z.string().min(1, "Código requerido").max(10, "Código muy largo"),
  ...
  gamma_humid: z.number().min(10, "γ húmedo >= 10 kN/m³").max(40, "γ húmedo <= 40 kN/m³"),
  gamma_saturated: z.number().min(10, "γ saturado >= 10 kN/m³").max(40, "γ saturado <= 40 kN/m³"),
  behavior_type: z.nativeEnum(BehaviorType),
  plasticity_index: z.number().min(0, "IP >= 0%").max(100, "IP <= 100%").optional(),
}).refine(
  (data) => data.gamma_saturated >= data.gamma_humid,
  {
  ...
  (data) => {
    // Plasticity index required for cohesive soils
    if (data.behavior_type === BehaviorType.COHESIVE && !data.plasticity_index) {
      return false;
    }
    return true;
  },
...
const strataDefinitionFormSchema = z.object({
  strata: z.array(stratumDefinitionSchema).min(1, "Al menos un tipo de estrato requerido")
}).refine(
  (data) => {
...
const StrataDefinitionForm: React.FC<StrataDefinitionFormProps> = ({ 
  ...
  const {
    ...
    defaultValues: {
      ...
      strata: Array.from({ length: projectData.number_of_strata }, (_, index) => ({
        ...
        behavior_type: BehaviorType.COHESIVE,
  ...
  useEffect(() => {
    const subscription = watch(() => {
      ...
      const strataCreateData: StratumCreate[] = formData.strata.map((stratum, index) => ({
  ...
  }, [watch, getValues, onValidData, isValid, projectData.id])
    ...
    <div className={styles.formContainer}>
      <div className={styles.formHeader}>
        <div className={styles.headerContent}>
          <div className={styles.titleSection}>
            ...
            <div>
              <h2 className={styles.formTitle}>Definición de Tipos de Estratos</h2>
      ...
      <div className={styles.infoCard}>
        ...
        <div className={styles.infoContent}>
          ...
          <p>
            Estos son los <strong>tipos de suelo</strong> que pueden aparecer en cualquiera de las 
            perforaciones del proyecto. En el siguiente paso, asignarás qué tipos aparecen y a qué 
            profundidades en cada perforación específica.
          </p>
        </div>
      </div>
          ...
          <div key={field.id} className={styles.stratumCard}>
            ...
            <div className={styles.cardContent}>
              <div className={styles.formRow}>
                <div className={styles.inputGroup}>
                  <label className={styles.label}>
                    Código del Estrato <span className={styles.required}>*</span>
                  </label>
                  <input
                    {...register(`strata.${index}.stratum_code`)}
                    className={`${styles.input} ${errors.strata?.[index]?.stratum_code ? styles.inputError : ''}`}
                ...
                <div className={styles.inputGroup}>
                  <label className={styles.label}>
                    Nombre del Estrato <span className={styles.required}>*</span>
                  </label>
                  <input
                    {...register(`strata.${index}.name`)}
                    className={`${styles.input} ${errors.strata?.[index]?.name ? styles.inputError : ''}`}
              ...
              <div className={styles.formRow}>
                <div className={styles.inputGroup}>
                  <label className={styles.label}>
                    Descripción del Suelo <span className={styles.required}>*</span>
                  </label>
                  <input
                    {...register(`strata.${index}.description`)}
                    className={`${styles.input} ${errors.strata?.[index]?.description ? styles.inputError : ''}`}
                ...
                <div className={styles.inputGroup}>
                  <label className={styles.label}>
                    Tipo de Comportamiento <span className={styles.required}>*</span>
                  </label>
                  <select
                    {...register(`strata.${index}.behavior_type`)}
                    className={`${styles.select} ${errors.strata?.[index]?.behavior_type ? styles.inputError : ''}`}
                  >
                ...
                {watch(`strata.${index}.behavior_type`) === BehaviorType.COHESIVE && (
                  <div className={styles.inputGroup}>
                    <label className={styles.label}>
                      Índice de Plasticidad (%) <span className={styles.required}>*</span>
                    ...
                    <input
                      type="number"
                      step="0.1"
                      {...register(`strata.${index}.plasticity_index`, { valueAsNumber: true })}
                      className={`${styles.input} ${errors.strata?.[index]?.plasticity_index ? styles.inputError : ''}`}
                      placeholder="15.0"
                    />
              ...
              <div className={styles.formRow}>
                <div className={styles.inputGroup}>
                  ...
                  <input
                    type="number"
                    step="0.1"
                    {...register(`strata.${index}.gamma_humid`, { valueAsNumber: true })}
                    className={`${styles.input} ${errors.strata?.[index]?.gamma_humid ? styles.inputError : ''}`}
                  />
                ...
                <div className={styles.inputGroup}>
                  <label className={styles.label}>
                    γ saturado (kN/m³) <span className={styles.required}>*</span>
                  ...
                  <input
                    type="number"
                    step="0.1"
                    {...register(`strata.${index}.gamma_saturated`, { valueAsNumber: true })}
                    className={`${styles.input} ${errors.strata?.[index]?.gamma_saturated ? styles.inputError : ''}`}
                  />
      ...
      <div className={styles.addStratumSection}>
        ...
        <span className={styles.addInfo}>
          Puedes agregar más tipos si en tu proyecto existen más variedades de suelo
        </span>
      </div>
          ...
          {fields.map((field, index) => (
            <div key={field.id} className={styles.summaryCard}>
              <div className={styles.summaryHeader}>
                <span className={styles.summaryCode}>{watch(`strata.${index}.name`)}</span>
                <span className={styles.summaryType}>{watch(`strata.${index}.behavior_type`)}</span>
              </div>
              ...
              <div className={styles.summaryProps}>
                ...
                <span>γsat: {watch(`strata.${index}.gamma_saturated`)} kN/m³</span>
